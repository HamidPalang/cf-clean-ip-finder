...
echo ""
echo -e "${B} Usable SNI Domains (???? ??????? ?? SNISPF):${N}"
if [[ ${#sorted_sni[@]} -eq 0 ]]; then
echo -e " ${R}??? ???????? ???? ???.${N}"
echo -e " ${Y}????????:${N}"
echo -e " ${Y}1. ???? ${CUSTOM_SNI_FILE} ?? ?? ????????? ????? ????? ????.${N}"
echo -e " ${Y}2. ???? ????? ?? ?? TLS ????? ???? (??????? ???? ???).${N}"
echo -e " ${Y}3. ?? ????? ???? ????? ??????? ???? (???? ??? ??? ????? ????????? ???? ?? ??????).${N}"
else
rank=1
...
echo -e " ${Y}Hint: ???? ??????? ?? ????????? ??????? ???? ${CUSTOM_SNI_FILE} ?? ????? ???? (?? ????? ?? ?? ??).${N}"
}
main "$@"